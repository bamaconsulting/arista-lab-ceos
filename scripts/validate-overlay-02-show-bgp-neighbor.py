...
def parse_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(
		description=(
			"Interactive EVPN BGP neighbor check: select device from inventory, "
			"discover neighbors from 'show bgp evpn summary', then run "
	...
	parser.add_argument("--neighbor", help="Neighbor IP (optional, skips interactive neighbor selection)")
...
def choose_device_interactively(hosts: list[tuple[str, str]]) -> tuple[str, str]:
	...
	selected_label = choose_option("Enter device number: ", options)
...
def parse_evpn_neighbors(summary_output: str) -> list[tuple[str, str, str]]:
	neighbors = []
	seen = set()
	...
	for line in summary_output.splitlines():
		...
		if ip not in seen:
			seen.add(ip)
			neighbors.append((description, ip, asn))
	return neighbors
...
def choose_neighbor_interactively(neighbors: list[tuple[str, str, str]]) -> str:
	print("\nDiscovered BGP neighbors:")
	options = []
	for index, (description, ip, asn) in enumerate(neighbors, start=1):
		label = f"{description} | {ip} | AS {asn}"
		options.append(label)
		print(f"[{index}] {label}")
	selected_label = choose_option("Enter BGP neighbor number: ", options)
	for description, ip, asn in neighbors:
		if selected_label == f"{description} | {ip} | AS {asn}":
			return ip
...
def main() -> int:
	...
	try:
		summary_response = node.enable(["show bgp evpn summary"], encoding="text")[0]
		summary_output = get_output(summary_response).rstrip()
	except Exception as exc:
		print(f"Execution error for 'show bgp evpn summary': {exc}")
		return 1

	print("\nOutput from 'show bgp evpn summary':")
	print(summary_output)

	neighbors = parse_evpn_neighbors(summary_output)
	if not neighbors:
		print("No BGP neighbors detected based on 'show bgp evpn summary'.")
		return 1

	neighbor_ips = [ip for _, ip, _ in neighbors]
	...
	primary_command = f"show bgp neighbor {neighbor_ip}"
	fallback_command = f"show bgp neighbors {neighbor_ip}"
	print(f"\nRunning: {primary_command}")

	try:
		response = node.enable([primary_command], encoding="text")[0]
		print("\nResult:")
		result_output = get_output(response).rstrip()
		print(highlight_matching_lines(result_output, HIGHLIGHT_TERMS))
		return 0
	except Exception as first_exc:
		print(f"Command failed, retrying with: {fallback_command}")
		try:
			response = node.enable([fallback_command], encoding="text")[0]
			print("\nResult:")
			result_output = get_output(response).rstrip()
			print(highlight_matching_lines(result_output, HIGHLIGHT_TERMS))
			return 0
		except Exception as second_exc:
			print(f"Execution error for '{primary_command}': {first_exc}")
			print(f"Execution error for '{fallback_command}': {second_exc}")
			return 1
