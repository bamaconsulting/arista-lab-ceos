...
def parse_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(description="Run 'show bgp evpn route-type mac-ip vni <VNI>' on selected devices via eAPI")
...
def highlight_mac_ip_mappings(output: str) -> str:
	...
	while index < len(lines):
		...
		if "RD:" in line and " mac-ip " in line:
			...
			while index < len(lines):
				...
				if stripped.startswith("*") and "RD:" in next_line and " mac-ip " in next_line:
					break
...
def main() -> int:
	...
	command = f"show bgp evpn route-type mac-ip vni {vni_id}"
	failures = 0
