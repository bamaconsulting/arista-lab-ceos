...
def parse_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(description="Run 'show ip bgp' on all FABRIC devices via eAPI")
...
def colorize_bgp_output(output: str) -> str:
	lines = []
...
def main() -> int:
	...
	for inventory_host, ip in hosts:
		...
		try:
			...
			response = node.enable(["show ip bgp"], encoding="text")[0]
			print(colorize_bgp_output(get_output(response).rstrip()))
		except Exception as exc:
			failures += 1
			print(f"ERROR: {exc}")
